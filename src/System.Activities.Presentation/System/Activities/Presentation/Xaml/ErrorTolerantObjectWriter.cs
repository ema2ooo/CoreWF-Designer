    ...
    internal class ErrorTolerantObjectWriter : XamlWriter, IXamlLineInfoConsumer
    {
        ...
        void ValidateStartObject()
        {
            ...
            if (type.IsUnknown)
            {
                ...
                {
                    ValidationError("Could not resolve generic type '{0}' with type arguments '{1}'.", GetXamlTypeName(type));
            ...
            else if (this.xamlStack.Count > 1)
            {
                ...
                if (!type.IsMarkupExtension)
                {
                    ...
                    if (collectionType != null)
                    {
                        ...
                        {
                            ValidationError("Object of type '{0}' cannot be added to collection '{1}'.", type, collectionType.ItemType, collectionType);
                        }
                    }
                    else if (parentMember != null && !parentMember.IsUnknown &&
                        !type.CanAssignTo(parentMember.Type) && parentMember.DeferringLoader == null)
                    {
                        ValidationError("Object of type '{0}' cannot be assigned to target '{1}'.", type, parentMember.Type, parentMember.Name);
                    }
                }
            }
        ...
        bool ValidateSetMember()
        {
            ...
            if (frame != null)
            {
                ...
                {
                    ValidationError("Member '{0}' can only be set once.", frame.Member);
                    return false;
                }
        ...
        void ValidateStartMember()
        {
            ...
            if (member == XamlLanguage.UnknownContent)
            {
                ...
                {
                    ValidationError("Unknown content '{0}' is not valid here.", this.xamlStack.Current.Type);
                }
            }
            else if (member.IsUnknown && (member.IsAttachable || member.IsDirective || !member.DeclaringType.IsUnknown))
            {
                ValidationError("Could not resolve member '{0}' on type '{1}'.", member.Name, member.DeclaringType);
            }
            ...
            {
                ValidationError("Member '{0}' has already been set on this object.", member);
            }
            else
            {
            ...
            {
                ValidationError("Member '{0}' cannot be set on target type '{1}'.", member.Name, member.TargetType);
            }
        ...
        void ValidateValue(object value)
        {
            ...
            if (member == XamlLanguage.Name || (type != null && member == type.GetAliasedProperty(XamlLanguage.Name)))
            {
                ...
                {
                    ValidationError("Duplicate name: '{0}'.", valueString);
                }
                return;
            }
            ...
            if (member == XamlLanguage.Initialization)
            {
                ...
                {
                    ValidationError("No type converter is available for '{0}'.", type);
                }
            }
            else if (member.IsDirective)
            {
                if (member == XamlLanguage.Items)
                {
                    ...
                    {
                        ValidationError("Collection '{0}' does not accept text content.", type);
                    }
                }
            }
            ...
            {
                ValidationError("Text of type '{0}' cannot be assigned to property '{2}' of type '{1}'.", XamlLanguage.String, member.Type, member.Name);
            }
        }
        ...
        void ValidationErrorUnknownType(XamlType type)
        {
            ...
            if (XamlNamespaceHelper.TryParseClrNsUri(type.PreferredXamlNamespace, out clrns, out assembly))
            {
                ...
                ValidationError("Could not resolve type '{0}' from assembly '{1}'.", typeName, assembly);
            }
            else
            {
                StringBuilder typeName = new StringBuilder();
                AppendShortName(typeName, type);
                ValidationError("Could not resolve type '{0}' in namespace '{1}'.", typeName, type.PreferredXamlNamespace);
            }
        }
        ...
        void CompleteNameReferences()
        {
            foreach (NameScopeFrame nameScope in this.poppedNameScopes)
            {
                ...
                foreach (NameReference reference in nameScope.NeededNames)
                {
                    ...
                    if (target == null)
                    {
                        ValidationError("Could not resolve name reference '{0}'.", reference.LineNumber, reference.LinePosition, reference.Name);
