...
namespace System.Activities.Presentation
{
    ...
    public partial class WorkflowDesigner
    {
        ...
        Exception lastDeserializeException;
        string lastLoadDiagnostics;
        ...
        public string LastLoadDiagnostics
        {
            get { return this.lastLoadDiagnostics ?? string.Empty; }
        }
...
So we suppress this error until the reporting for CA3053 has been updated to fix this issue.")]
        ...
        public void Load()
        {
            ...
            this.lastDeserializeException = null;
            this.lastLoadDiagnostics = string.Empty;

            if (!string.IsNullOrEmpty(this.text))
            {
                IList<XamlLoadErrorInfo> loadErrors = null;
                Dictionary<object, SourceLocation> sourceLocations = null;
                object deserializedObject = null;
                List<string> assemblyResolutionFailures = new List<string>();
                List<string> typeResolutionFailures = new List<string>();
                ResolveEventHandler assemblyResolveHandler = (sender, args) =>
                {
                    assemblyResolutionFailures.Add(args.Name);
                    return null;
                };
                ResolveEventHandler typeResolveHandler = (sender, args) =>
                {
                    typeResolutionFailures.Add(args.Name);
                    return null;
                };
                ...
                try
                {
                    try
                    {
                        ...
                        deserializedObject = DeserializeString(this.text, out loadErrors, out sourceLocations);
                        ...
                        {
                            string details = this.BuildLoadFailureDetails(loadErrors, assemblyResolutionFailures, typeResolutionFailures, null, true);
                            this.lastLoadDiagnostics = details;
                            this.Context.Items.SetValue(new ErrorItem() { Message = SR.SeeErrorWindow, Details = details });
                            System.Diagnostics.Trace.TraceError(details);
                        }
                    ...
                    {
                        string details = this.BuildLoadFailureDetails(loadErrors, assemblyResolutionFailures, typeResolutionFailures, e, false);
                        this.lastLoadDiagnostics = details;
                        this.Context.Items.SetValue(new ErrorItem() { Message = e.Message, Details = details });
                        System.Diagnostics.Trace.TraceError(details);
                        RaiseLoadError(e);
                    }
                }
        ...
        void RecordDeserializeException(Exception exception)
        {
            if (exception != null)
            {
                this.lastDeserializeException = exception;
            }
        }

        string BuildLoadFailureDetails(IList<XamlLoadErrorInfo> loadErrors, IList<string> assemblyResolutionFailures, IList<string> typeResolutionFailures, Exception loadException, bool deserializedObjectWasNull)
        {
            StringBuilder details = new StringBuilder();
            if (!string.IsNullOrWhiteSpace(this.loadedFile))
            {
                details.AppendLine(string.Format(CultureInfo.CurrentCulture, "Workflow file: {0}", this.loadedFile));
            }

            if (deserializedObjectWasNull)
            {
                details.AppendLine("XAML deserialization returned null.");
            }

            if (this.lastDeserializeException != null)
            {
                details.AppendLine("Initial deserializer exception:");
                details.AppendLine(this.lastDeserializeException.ToString());
            }

            if (loadException != null)
            {
                details.AppendLine("Load exception:");
                details.AppendLine(loadException.ToString());
            }

            if (loadErrors != null && loadErrors.Count > 0)
            {
                details.AppendLine("XAML load errors:");
            ...
            {
                details.AppendLine("XAML load errors: none were reported by the tolerant loader.");
            }

            AppendResolutionFailures(details, "Assembly resolution failures", assemblyResolutionFailures);
            AppendResolutionFailures(details, "Type resolution failures", typeResolutionFailures);
            AppendMissingReferencedAssemblies(details);
            AppendResolutionExceptions(details, this.lastDeserializeException);
            AppendResolutionExceptions(details, loadException);

            return details.ToString().Trim();
        }

        static void AppendResolutionFailures(StringBuilder details, string title, IList<string> failures)
        {
            ...
            foreach (string failure in failures.Where(s => !string.IsNullOrWhiteSpace(s)).Distinct(StringComparer.OrdinalIgnoreCase))
            {
                details.AppendLine("- " + failure);
            }
        }

        void AppendMissingReferencedAssemblies(StringBuilder details)
        {
            foreach (string assemblyName in GetReferencedAssemblyNames(this.text))
            {
                if (!IsLikelyCustomAssembly(assemblyName))
                {
                    continue;
                }
                ...
                {
                    details.AppendLine(string.Format(CultureInfo.CurrentCulture, "Missing custom activity assembly: {0} ({1}: {2})", assemblyName, ex.GetType().Name, ex.Message));
                }
            }
        }

        static IEnumerable<string> GetReferencedAssemblyNames(string xamlText)
        {
            ...
            MatchCollection matches = Regex.Matches(xamlText, @"assembly=([^;""\s]+)", RegexOptions.IgnoreCase);
        ...
        static bool IsLikelyCustomAssembly(string assemblyName)
        {
        ...
        static void AppendResolutionExceptions(StringBuilder details, Exception exception)
        {
            if (exception == null)
            {
                return;
            ...
            foreach (Exception current in FlattenExceptions(exception))
            {
                if (current is FileNotFoundException || current is FileLoadException || current is TypeLoadException)
                {
                    details.AppendLine(string.Format(CultureInfo.CurrentCulture, "Resolution exception: {0}: {1}", current.GetType().Name, current.Message));
                }
            }
        }
        ...
        static IEnumerable<Exception> FlattenExceptions(Exception exception)
        {
            ...
            while (current != null)
            {
                ...
                if (current is ReflectionTypeLoadException reflectionTypeLoadException && reflectionTypeLoadException.LoaderExceptions != null)
                {
